...
class Settings(BaseSettings):
    ...
    msf_host: str = Field(
        default="127.0.0.1",
        description="Metasploit RPC server hostname or IP address"
    )
    msf_port: int = Field(
        default=55553,
        ge=1,
    ...
    rate_limit_enabled: bool = Field(
        default=True,
        description="Enable rate limiting for RPC calls"
    )
    ...
    auth_mode: AuthMode = Field(
        ...
        description="Authentication mode for MCP clients"
    )
    auth_token: Optional[SecretStr] = Field(
        default=None,
        description="Authentication token for MCP clients (if auth_mode=token)"
    )
    allowed_modules: Optional[list[str]] = Field(
        default=None,
        description="Whitelist of allowed Metasploit modules (None = all allowed)"
    )
    blocked_modules: list[str] = Field(
        default_factory=list,
        description="Blacklist of blocked Metasploit modules"
    )
    max_concurrent_sessions: int = Field(
        default=10,
        ge=1,
        le=100,
        description="Maximum concurrent Meterpreter/shell sessions"
    )
    ...
    # Feature Flags
    enable_exploit_tools: bool = Field(
        default=True,
        description="Enable exploit execution tools"
    )
    enable_payload_tools: bool = Field(
        default=True,
        description="Enable payload generation tools"
    )
    enable_auxiliary_tools: bool = Field(
        default=True,
        description="Enable auxiliary module tools"
    )
    enable_post_tools: bool = Field(
        default=True,
        description="Enable post-exploitation tools"
    )
    enable_session_tools: bool = Field(
        default=True,
        description="Enable session management tools"
    )
    enable_db_tools: bool = Field(
        default=True,
        description="Enable database/workspace tools"
    )

    # Safety Settings
    require_confirmation: bool = Field(
        default=False,
        description="Require confirmation before executing exploits"
    )
    dry_run_mode: bool = Field(
        default=False,
        description="Enable dry-run mode (no actual exploitation)"
    )
    audit_logging: bool = Field(
        default=True,
        description="Enable detailed audit logging of all operations"
    )
    ...
    @field_validator("blocked_modules", mode="before")
    @classmethod
    def parse_blocked_modules(cls, v: str | list[str]) -> list[str]:
        """Parse blocked modules from comma-separated string or list."""
        if isinstance(v, str):
